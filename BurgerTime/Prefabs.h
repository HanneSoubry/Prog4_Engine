...
namespace dae
{
	...
	{
	public:
		static std::unique_ptr<GameObject> Create(const std::string& texture, const glm::vec2& startPosition, float moveSpeed, const std::vector<unsigned int>& moveKeys, int controllerId = -1)
		{
			glm::vec2 size{};
			...
			InputManager::GetInstance().BindCommand(moveKeys, InputManager::InputAction::Digital2DAxis,
				std::move(command), controllerId);

			return std::move(player);
		}

		static std::unique_ptr<GameObject> Create(const std::string& texture, const glm::vec2& startPosition, float moveSpeed, unsigned int moveKey, int controllerId)
		{
			glm::vec2 size{};
			auto player = BasePlayer(texture, startPosition, size);

			std::unique_ptr<Command<glm::vec2>> command{ std::move(std::make_unique<MoveOnGridCommand>(MoveOnGridCommand{ player.get(), moveSpeed, size })) };
			InputManager::GetInstance().BindCommand(moveKey, InputManager::InputAction::Digital2DAxis,
				std::move(command), controllerId);
